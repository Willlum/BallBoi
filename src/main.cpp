...
void setup() {
  ...
  cmd.addCommand("M1", setMotorRPM);
  cmd.addCommand("S1", moveStepper);
...
void loop() {
  while(Serial2.available()){ //use rx interrupt handler
...
void moveStepper(){
  ...
  char* garbage = NULL;
  if(arg != NULL){
    targetStepCount = (uint32_t) strtol(arg, &garbage,0);
  }
  #ifdef DEBUG_SERIAL
